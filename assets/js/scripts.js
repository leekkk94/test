	...
	function _topNav() {
		...
		jQuery('li.search i.fa').click(function () {
			if(jQuery('#header .search-box').is(":visible")) {
				...
				if (jQuery('#header li.quick-push div.quick-push-box').is(":visible")) {
					jQuery('#header li.quick-push div.quick-push-box').fadeOut(300);
				}
			}
		});
		...
		// Quick push
		jQuery('li.quick-push>a').click(function (e) {
			e.preventDefault();

			var _quick_push_box = jQuery('li.quick-push div.quick-push-box');

			if(_quick_push_box.is(":visible")) {
				_quick_push_box.fadeOut(300);
				jQuery('#wrapper').removeClass('fixed');
			} else {
				//jQuery('#wrapper').addClass('fixed');
				_quick_push_box.fadeIn(300);


			}
		});
		// Quick push close
		jQuery('.quick-push-footer>#quick-push-close').click(function (e) {
			e.preventDefault();
		...
		if(jQuery('li.quick-push>a').size() != 0) {
			jQuery('li.quick-push').on('click', function(e){
				e.stopPropagation();
			});

			jQuery('body').on('click', function() {
				if (jQuery('li.quick-push div.quick-push-box').is(":visible")) {
						jQuery('li.quick-push div.quick-push-box').fadeOut(300);
						jQuery('#wrapper').removeClass('fixed');
				}
			});
		}
		...
		jQuery(window).scroll(function() {
			if(window.width < 769) {
				// hide quick cart if visible
				// if (jQuery('#header li.quick-push div.quick-push-box').is(":visible")) {
				// 	jQuery('#header li.quick-push div.quick-push-box').fadeOut(0);
				// }
